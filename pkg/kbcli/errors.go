...
func makeComponentNotExistErr(clusterName, component string) error {
	return fmt.Errorf(componentNotExistErrMessage, clusterName, component)
}

func makeConfigSpecNotExistErr(clusterName, component, configSpec string) error {
	return fmt.Errorf(notFoundConfigSpecErrorMessage, configSpec, component, clusterName)
}

func makeNotFoundConfigFileErr(configFile, configSpec string, all []string) error {
	return fmt.Errorf(notFoundConfigFileErrorMessage, configFile, configSpec, all)
}

func makeMissingClusterNameErr() error {
	return fmt.Errorf("%s", missingClusterArgErrMassage)
}
