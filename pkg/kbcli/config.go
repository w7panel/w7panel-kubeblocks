...
func (w *ReconfigureWrapper) ConfigSpecName() string {
	...
	if file != "" && w.rctx.ConfigRender != nil {
		config := getComponentConfigDescription(&w.rctx.ConfigRender.Spec, file)
		if config != nil {
...
func GetClientFromRestConfig(config *rest.Config) versioned.Interface {
...
func resolveComponentConfigRender(rctx *ReconfigureContext, cmpd *kbappsv1.ComponentDefinition) (*parametersv1alpha1.ParamConfigRenderer, error) {
	...
	if len(prcs) > 1 {
		return nil, fmt.Errorf("the ParamConfigRenderer is ambiguous which referenced cmpd[%s], prcs: [%s]", cmpd.Namespace,
			strings.Join(mapParamConfigRendererNames(prcs), ", "))
	}
	return nil, nil
}
...
func getComponentConfigDescription(pdcr *parametersv1alpha1.ParamConfigRendererSpec, name string) *parametersv1alpha1.ComponentConfigDescription {
	for i := range pdcr.Configs {
		if pdcr.Configs[i].Name == name {
			return &pdcr.Configs[i]
		}
	}
	return nil
}

func mapParamConfigRendererNames(items []parametersv1alpha1.ParamConfigRenderer) []string {
